...
def neural_net(learn_gain,rand_rate):
    ...
    print "rand ratio is "+ str(rand_rate)
    test_num = 10000
    correct_count = 0
    count = 0
...
'''rand_ratio_step = 0.01
learn_count = int((max_rand_ratio - rand_ratio) / rand_ratio_step + 1)
#print learn_count
...
for var in range(0,learn_count):
    for a in range(0,learn_count_single):
        tmp = neural_net(0.1,rand_ratio)
        print tmp
        result[var] += tmp
    rand_ratio += rand_ratio_step
